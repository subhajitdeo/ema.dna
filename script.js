...
function renderCards() {
    ...
    if (filteredData.length === 0) {
        container.innerHTML = `<div style="color:#94A3B8; text-align:center; padding:40px; font-size:18px;">🔍 No results found for "${searchInput.value}"</div>`;
        return;
    }
    filteredData.forEach(stock => container.appendChild(createCard(stock)));
}
...
function applyFilterAndSort() {
    ...
    if (query === "") {
        ...
        filteredData = currentData.filter(stock => 
            stock.symbol.toLowerCase().includes(query)
        );
    }
    // Re-apply sorting
    const criterion = sortSelect.value;
    if (criterion === "score") filteredData.sort((a, b) => b.score - a.score);
    else if (criterion === "change") filteredData.sort((a, b) => b.change - a.change);
    else if (criterion === "symbol") filteredData.sort((a, b) => a.symbol.localeCompare(b.symbol));
    renderCards();
}

function sortData(criterion) {
    // Just trigger the combined filter+sort
    applyFilterAndSort();
}
...
async function loadData() {
    try {
        ...
        console.error(err);
        updateBar.innerText = "● Error loading data. Run scanner.py first.";
        container.innerHTML = "<div style='color:red; text-align:center'>Could not load data/results.json</div>";
    }
}
